...
void sortMonthlyInvoicebyRent(tRentInvoiceData* list) {
    ...
    //LIST INLUDES JAN - FEB - MAR //
    tRentInvoiceMonthly* monthlyInvoices[list->count];
    
    //LIST INCLUDES 150-250-270)
    float totalRents[list->count];

    //COPY NODES TO LINKED LIST && CALCULATE INVOICE //
    ...
    //IF CURRENT ELEMENT IS THE HIGHEST//
    for(int i = 0; i < list->count - 1; i++) {
        ...
        //GIVEN THE ORDERED ARRAY, LINK IT TO THE LIST//
        //FIRST NODE AS FIRST ARRAY POSSITION//
        list->first = monthlyInvoices[0]; 
        for(int i = 0; i < list->count - 1; i++) {
            
            //ITERATE THROUGH NEXT NODES IN THE ARRAY//
            monthlyInvoices[i]->next = monthlyInvoices[i + 1];
        }
        
        //LAST NODE SET TO NULL//
        monthlyInvoices[list->count - 1]->next = NULL;
    }
}
...
void sortMonthlyInvoicebyDate(tRentInvoiceData* list) {
    ...
    //IF LIST IS ORDERED, EMPTY OR HAS ONLY 1 ELEMENT//
    if (list == NULL || list->first == NULL || list->first->next == NULL) {
    return; 
    }

    //ASIGN POINTERS TO SORTED LIST//
    tRentInvoiceMonthly* sorted = NULL;
    tRentInvoiceMonthly* current = list->first;

    //GET VALUE FROM CURRENT NODE PROCEDURE//
    while (current != NULL) {
        tRentInvoiceMonthly* next = current->next;

        //INSERT CURRENT NODE IN LIST//
        if (sorted == NULL || date_cmp(current->month, sorted->month) < 0) {
            //IF EMPTY LIST OR OLDEST DATE -> INSERT IN FIRST POSSITION//
            current->next = sorted;
            sorted = current;
        } else {
            
            //INSERT IN THE SORTED POSSITION//
