...
tApiError api_getMonthlyInvoiceByCadastralRef(tApiData data, char* cadastral_ref ,tCSVData *invoices) {
    ...
    //EX: “01;02;2020;1000.00;ABC1234”//
    // “dd;mm;yyyy;rent;cadastral_ref”//
    ...
    while(pNode != NULL) {
        ...
        while(pMonthly != NULL) {
            ...
            while(pNode != NULL) {
                sprintf(buffer, "%02d;%02d;%d;%.2f;%s", pMonthly->month.day, pMonthly->month.month, pMonthly->month.year, pNode->elem.rent, pNode->elem.cadastral_ref);
...
tApiError api_getMonthlyInvoiceByCadastralRef_Reverse(tApiData data, char* cadastral_ref ,tCSVData *invoices) {
   ...
   char buffer[2048];
    ...
    pNode = data.taxOffices.first;
    while(pNode != NULL) {
        tRentInvoiceData sortedList = findMonthlyInvoiceByCadastralRef_Reverse(&(pNode->elem.rentInvoices), cadastral_ref);
        tRentInvoiceMonthly* pMonthly = sortedList.first;
        while(pMonthly != NULL) {
            tRentInvoiceNode* pNode = pMonthly->first;
            while(pNode != NULL) {
...
tApiError api_getAddressByCadastralRef (tApiData data, char* cadastral_ref, tCSVEntry *address) {
    ...
    //EX: Privet Drive; 4" "street;number//
...
tApiError api_findLatestTenant(tApiData data, char* cadastral_ref, tCSVEntry *tenant ) {
    ...
    //EX: // "01/01/2024;31/12/2024;87654321B;Jason;750.0;30;ZYX1234;BA08002\n"//
    //  FORMAT AS     //
    //EX: "dd/mm/yyyy;dd/mm/yyyy;tenant_id;name;rent;age;cadastral_ref;postal_code"//
    char buffer[2048];
    tTenant* pTenant = NULL;
    int idx = findLatestTenant(&data.tenants, cadastral_ref);
    if(idx >= 0) {
        pTenant = &(data.tenants.elems[idx]);
        sprintf(buffer, "%02d/%02d/%d;%02d/%02d/%d;%s;%s;%.2f;%d;%s;%s", pTenant->start_date.day, pTenant->start_date.month, pTenant->start_date.year, pTenant->end_date.day, pTenant->end_date.month, pTenant->end_date.year, pTenant->tenant_id, pTenant->name, pTenant->rent, pTenant->age, pTenant->cadastral_ref, pTenant->postal_code); csv_parseEntry(tenant, buffer, "TENANT");
    }
    return E_SUCCESS;
}
...
tApiError api_findFirstTenant(tApiData data, char* cadastral_ref, tCSVEntry *tenant) {
    ...
    if(idx >= 0) {
        pTenant = &(data.tenants.elems[idx]);
        sprintf(buffer, "%02d/%02d/%d;%02d/%02d/%d;%s;%s;%.2f;%d;%s;%s", pTenant->start_date.day, pTenant->start_date.month, pTenant->start_date.year, pTenant->end_date.day, pTenant->end_date.month, pTenant->end_date.year, pTenant->tenant_id, pTenant->name, pTenant->rent, pTenant->age, pTenant->cadastral_ref, pTenant->postal_code);
