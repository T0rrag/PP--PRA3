...
tApiError api_addTenant(tApiData* data, tCSVEntry entry)
{
    ...
    invoiceList_update(
        &pTaxOffice->rentInvoices, tenant.start_date, tenant.end_date, tenant.cadastral_ref, tenant.rent);
...
tApiError api_getRentalIncome(tApiData data, int year, const char* id, tCSVEntry* entry)
{
    ...
    // Print data in the buffer
    sprintf(buffer, "%d;%.1f;%s", rentalIncome->year, rentalIncome->totalIncome, rentalIncome->landlord->id);
    // Initialize the output structure
    csv_initEntry(entry);
...
tApiError api_getProperties(tApiData data, tCSVData* properties)
{
    ...
    for(int i = 0; i < data.landlords.count; i++) {
        for(int j = 0; j < data.landlords.elems[i].properties.count; j++) {
            ...
            sprintf(buffer, "%s;%s;%d;%s", property->cadastral_ref, property->address.street, property->address.number,
...
tApiError api_getMonthlyInvoiceByCadastralRef(tApiData data, char* cadastral_ref, tCSVData* invoices)
{
    // PR3 4a
    char buffer[2048];
    tTaxOfficeNode* pNode = NULL;
    csv_init(invoices);

    pNode = data.taxOffices.first;

    while(pNode != NULL) {
        tRentInvoiceData sortedList = findMonthlyInvoiceByCadastralRef(&(pNode->elem.rentInvoices), cadastral_ref);
        tRentInvoiceMonthly* pMonthly = sortedList.first;
        while(pMonthly != NULL) {
            tRentInvoiceNode* pNode = pMonthly->first;
            while(pNode != NULL) {
                // Ejemplo: “01;02;2020;1000.00;ABC1234” => “dd;mm;yyyy;rent;cadastral_ref”
                sprintf(buffer, "%02d;%02d;%d;%.2f;%s", pMonthly->month.day, pMonthly->month.month,
                    pMonthly->month.year, pNode->elem.rent, pNode->elem.cadastral_ref);

                csv_addStrEntry(invoices, buffer, "INVOICE");
                pNode = pNode->next;
            }
            pMonthly = pMonthly->next;
        }
        pNode = pNode->next;
        invoiceList_free(&sortedList);
    }
...
tApiError api_getMonthlyInvoiceByCadastralRef_Reverse(tApiData data, char* cadastral_ref, tCSVData* invoices)
{
    ...
    while(pNode != NULL) {
        tRentInvoiceData sortedList =
            findMonthlyInvoiceByCadastralRef_Reverse(&(pNode->elem.rentInvoices), cadastral_ref);
        tRentInvoiceMonthly* pMonthly = sortedList.first;
        while(pMonthly != NULL) {
            tRentInvoiceNode* pNode = pMonthly->first;
            while(pNode != NULL) {
...
tApiError api_getMonthlyInvoicebyRent(tApiData data, char* office_code, tCSVData* invoices)
{
    // PR3 4c
    char buffer[2048];
    tTaxOffice* pTaxOffice = NULL;
    csv_init(invoices);

    pTaxOffice = taxOfficeList_find(&(data.taxOffices), office_code);

    if(pTaxOffice != NULL) {
        tRentInvoiceData sortedList = pTaxOffice->rentInvoices;
        sortMonthlyInvoicebyRent(&sortedList);
        tRentInvoiceMonthly* pMonthly = sortedList.first;
        while(pMonthly != NULL) {
            tRentInvoiceNode* pNode = pMonthly->first;
            while(pNode != NULL) {
...
tApiError api_getAddressByCadastralRef(tApiData data, char* cadastral_ref, tCSVEntry* address)
{
    // PR3 4d
    char buffer[2048];
    csv_initEntry(address);
    int idx = landlords_find_by_cadastral_ref(data.landlords, cadastral_ref);

    if(idx >= 0) {

        int idxProperty = properties_find(data.landlords.elems[idx].properties, cadastral_ref);
        tProperty* property = &(data.landlords.elems[idx].properties.elems[idxProperty]);
        // “Privet Drive;4” => “street;number”
        sprintf(buffer, "%s;%d", property->address.street, property->address.number);
        csv_parseEntry(address, buffer, "ADDRESS");
    }
...
{
    // PR3 4e
    sortLandlordsByName_QuickSort(&(data->landlords));
...
tApiError api_findLatestTenant(tApiData data, char* cadastral_ref, tCSVEntry* tenant)
{
    // PR3 4f
    char buffer[2048];
    tTenant* pTenant = NULL;
    int idx = findLatestTenant(&data.tenants, cadastral_ref);
    if(idx >= 0) {
        pTenant = &(data.tenants.elems[idx]);
        // "01/01/2024;31/12/2024;87654321B;Jason;750.0;30;ZYX1234;BA08002\n"  =>
        // "dd/mm/yyyy;dd/mm/yyyy;tenant_id;name;rent;age;cadastral_ref;postal_code"
        sprintf(buffer, "%02d/%02d/%d;%02d/%02d/%d;%s;%s;%.2f;%d;%s;%s", pTenant->start_date.day,
            pTenant->start_date.month, pTenant->start_date.year, pTenant->end_date.day, pTenant->end_date.month,
            pTenant->end_date.year, pTenant->tenant_id, pTenant->name, pTenant->rent, pTenant->age,
            pTenant->cadastral_ref, pTenant->postal_code);
        csv_parseEntry(tenant, buffer, "TENANT");
    }
...
tApiError api_findFirstTenant(tApiData data, char* cadastral_ref, tCSVEntry* tenant)
{
    ...
    char buffer[2048];
    tTenant* pTenant = NULL;
    int idx = findFirstTenant(&data.tenants, cadastral_ref);
    if(idx >= 0) {
        pTenant = &(data.tenants.elems[idx]);
        // "01/01/2024;31/12/2024;87654321B;Jason;750.0;30;ZYX1234;BA08002\n"  =>
        // "dd/mm/yyyy;dd/mm/yyyy;tenant_id;name;rent;age;cadastral_ref;postal_code"
...
{
    // PR3 4h
    sortTenantsById_Bubble(&(data->tenants));
...
tApiError api_getRentalIncomes(tApiData data, tCSVData* rentalIncomes)
{
    ...
    while(pNode != NULL) {
        sprintf(buffer, "%d;%.2f;%s", pNode->elem.year, pNode->elem.totalIncome, pNode->elem.landlord->id);
