...
int findLatestTenant(tTenantData *data, char* cadastral_ref) {
    ...
    //data stream as//
    //NAME - CADASTRAL REF - END DATE//
    
    //RETURN LAS FOUND TENANT//
    int latestIndex = -1;
    tDate latestEndDate = {0, 0, 0};
    

    //ITERATE THROUGH TENATS//
    for(int i = 0; i < data->count; i++) {
        
        //MATCH CADASTRAL REF// 
        if(strcmp(data->elems[i].cadastral_ref, cadastral_ref) == 0) {
            
            //IF FOUND WITH NON VALUD END DATE//
            if(date_cmp(data->elems[i].end_date, latestEndDate) > 0) {
                
                //UPDATE LATEST DATE && UPDATE LATEST INDEX//
                latestEndDate = data->elems[i].end_date; 
                latestIndex = i;
            }
        }
    }
...
int findFirstTenant(tTenantData *data, char* cadastral_ref){
    ...
    //GET FIRST FOUND TENANT INDEX // 
    int firstIndex = -1;
    
    //STORE ELDEST DATE//
    tDate earliestStartDate;
    ...
    for(int i = 0; i < data->count; i++) {
        if(strcmp(data->elems[i].cadastral_ref, cadastral_ref) == 0) {
            if(firstIndex == -1) {
                ...
                //UPDATE INDEX IF EARLIER DATE FOUND//
                firstIndex = i; 
